...
func initGraphClient(ctx context.Context, cfg Config) *msgraphsdk.GraphServiceClient {
	...
	adapter, err := msgraphsdk.NewGraphRequestAdapterWithCreds(cred)
	if err != nil {
...
func userExists(ctx context.Context, cfg Config, client *msgraphsdk.GraphServiceClient, email string) bool {
	...
	if err != nil {
		if respErr, ok := err.(*msusers.ODataError); ok && respErr.StatusCode() == http.StatusNotFound {
			return false
		}
	...
	for _, ns := range nsList.Items {
		...
		if !userExists(ctx, cfg, graph, email) {
			...
			if cfg.DryRun {
				...
				_, err := kube.CoreV1().Namespaces().Patch(ctx, ns.Name, metav1.TypeMergePatchType, patch, metav1.PatchOptions{})
				if err != nil {
